...
# Identify and describe the Ruby method(s) you implemented.
#For the array method I used delete_if? and .include?. 
#Delete_if uses a block to determine what should be removed from the array, 
#and in that block I turned all the elements to strings so they could be checked 
#and then used .include to check all the strings for the offending character and 
#let the method know which ones to remove.
#
For the hash method I also used delete_if. This one was simpler because the 
#block was just comparing the key in the hash to the one passed to it.

#I want to note that I told it to p source because these methods are destructive, 
#so they array and hash would be permanently altered, but by themselves the methods 
#return the deleted value, so you have to specify that you want the method to return
#the result of deleting the value instead.
...
# and delete_if for hashes. 
# Delete_if uses a block to determine what to remove from the source. 
# The block that I passed to delete_if for the array turned all the elements into 
# strings (nondestructively) using to_s, because the .include method can be used 
# with strings but not with integers. .include checks to see which strings include 
# the string passed to it. Delete_if will then remove the strings that .include finds. 

# For the hash, I used the block to compare each key in the hash to the 
...
# It should be kept in mind what I mentioned above: delete_if will remove what 
# you want it to remove, but then it returns that, so make sure to specify that 
# you want the changed array removed.

# 3. Share any tricks you used to find and decipher the Ruby Docs
# I'm a ctrl+f'er, too, Shawn! I'll go into the section I know I need to look through and 
# then search for keywords. I usually have an idea of how ruby "talks" so sometimes I can 
# land on the method just by searching for what I want it to do. That was how I found include right away.
